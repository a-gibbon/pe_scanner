...
def conv(s):
    return "".join([ to_str(c) for c in s ])
...
class PeScan:
    def __init__(self, name=None, data=None):
        ...
        self.PDB_PATH = conv(pe.get_string_from_data(0x18, data))
        ...
        if hasattr(pe, 'DIRECTORY_ENTRY_IMPORT'):
            for library in pe.DIRECTORY_ENTRY_IMPORT:
                DLL = conv(library.dll.upper())
                APIs = []
                for API in library.imports:
                    if API.name:
                        APIs.append(conv(API.name))
        ...
        if hasattr(pe, 'VS_VERSIONINFO') and hasattr(pe, 'FileInfo'):
            ...
            for key, val in entries.items():
                version_information.append([conv(key), conv(val)])
